...
wchar_t g_fsr_easu_shader_path[MAX_PATH] = {};
wchar_t g_fsr_rcas_shader_path[MAX_PATH] = {};
...
BOOL CALLBACK initialize_paths(PINIT_ONCE, PVOID, PVOID*) {
    ...
    std::wcsncpy(g_overlay_shader_path, g_plugin_root, MAX_PATH - 1);
    append_path(g_overlay_shader_path, MAX_PATH, L"\\shaders\\overlay.hlsl");

    std::wcsncpy(g_fsr_easu_shader_path, g_plugin_root, MAX_PATH - 1);
    append_path(
        g_fsr_easu_shader_path, MAX_PATH, L"\\shaders\\fsr_easu.hlsl");

    std::wcsncpy(g_fsr_rcas_shader_path, g_plugin_root, MAX_PATH - 1);
    append_path(
        g_fsr_rcas_shader_path, MAX_PATH, L"\\shaders\\fsr_rcas.hlsl");
...
const wchar_t* bloom_shader_path() {
    ensure_paths();
    return g_bloom_shader_path;
}

const wchar_t* overlay_shader_path() {
    ensure_paths();
    return g_overlay_shader_path;
}

const wchar_t* fsr_easu_shader_path() {
    ensure_paths();
    return g_fsr_easu_shader_path;
}

const wchar_t* fsr_rcas_shader_path() {
    ensure_paths();
    return g_fsr_rcas_shader_path;
}
